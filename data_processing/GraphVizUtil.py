...
def processInput(args):
    ...
    parser.add_argument('-i', '--input', action="store", dest="ifile",
                        help="Input Database Filename <path>", required=True)
    parser.add_argument('-f', '--format', action="store", dest="image_format",
                        help="Graphviz output format (Default: svg)", default='svg')
    ...
    parser.add_argument('-v', '--splinevar', action="store_true", dest="splinevar",
                        help="Whether subgraph edges to be straight or curved (Default: Straight)", default=False)
    ...
    parser.add_argument('-g', '--gray', action="store_true", dest="grey_flag",
                        help="If specified, generates graph in graycale", default=False)
    parser.add_argument('-n', '--name', action="store", dest="quick_name",
                        help="Specify the extension you wish to give your quick run")
    parser.add_argument('-o', '--output', action="store", dest="res_dir",
                        help='Optional output file path (to dump the images folder)', default='./')

    group1 = parser.add_mutually_exclusive_group()
    group1.add_argument('-b', '--technology', action="store", dest="inp_technology",
                        help="Technology for which graph to be generated")
    group1.add_argument('-a', '--commodity', action="store", dest="inp_commodity",
                        help="Commodity for which graph to be generated")

    parser.add_argument('-s', '--scenario', action="store", dest="scenario_name",
                        help="Model run scenario name", default=None)
    parser.add_argument('-y', '--year', action="store", dest="period", type=int,
    ...
    parser.add_argument('-r', '--region', action="store", dest="region",
                        help="The region for which the graph is to be generated", default=None)
