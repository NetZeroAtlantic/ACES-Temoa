    ...
    def solveWithMGA(self):
        ...
        while self.options.next_mga():
            ...
            temoaMGAInstance.instance.PreviousSlackedObjective = Constraint(
                rule=None,
                expr=SlackedObjective_rule(temoaMGAInstance.instance,
                                           Perfect_Foresight_Obj, self.options.mga),
                noruleinit=True
            )
...
class TemoaSolverInstance(object):
    ...
    def create_temoa_instance(self):
        ...
        try:
            ...
            begin = time()
            def duration(): return time() - begin
    ...
    def handle_files(self, log_name):
        ...
        if isinstance(self.options, TemoaConfig) and self.options.saveTEXTFILE:
            ...
            new_dir = self.options.path_to_data+os.sep + \
                file_ty.group(1)+'_'+self.options.scenario+'_model'
            if path.isfile(self.options.path_to_logs+os.sep+log_name) and path.exists(new_dir):
                copyfile(self.options.path_to_logs+os.sep+log_name, new_dir +
                         os.sep+self.options.scenario+'_OutputLog.log')
        ...
        if isinstance(self.options, TemoaConfig) and self.options.keepPyomoLP:
            ...
            if path.exists(new_dir):
                move(self.options.path_to_lp_files+os.sep+lpfile,
...
def parse_args():
    ...
    parser.add_argument('--path_to_logs',
                        ...
                        action='store',
                        dest='path_to_logs',
                        default=re.sub('temoa_model$', 'data_files', dirname(
