...
def pformat_results(pyomo_instance, pyomo_result, options):
    ...
    for r, p, s, d, i, t, v, o in m.V_Curtailment:
        ...
        svars['V_FlowIn'][r, p, s, d, i, t, v, o] = (
            val + value(m.V_FlowOut[r, p, s, d, i, t, v, o])) / value(m.Efficiency[r, i, t, v, o])
    ...
    if isinstance(options, TemoaConfig):
        ...
        # WRITE DUALS RESULTS
        overwrite_keys = [str(tuple(x)) for x in duals.reset_index()[
            ['constraint_name', 'scenario']].to_records(index=False)]
        # delete records that will be overwritten by new duals dataframe
        cur.execute("DELETE FROM Output_Duals WHERE (constraint_name, scenario) IN (VALUES " +
                    ','.join(overwrite_keys) + ")")
