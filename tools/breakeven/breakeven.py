...
def sensitivity(dat, techs):
    ...
    for t in techs:
        ...
        print("{:>10s}\t{:>7s}\t{:>6s}\t{:>4s}\t{:>6s}\t{:>5s}\t{:>7s}\t{:>7s}\t{:>5s}\t{:>3s}\t{:>5s}".format(
            'Tech', 'Vintage', 'L. RC', 'Coef', 'U. RC', 'Scale', 'BE IC', 'BE FC', 'IC', 'FC', 'Cap'))
        for v in vintages:
    ...
    for e in instance.commodity_emissions:
        for p in instance.time_optimize:
            if (p, e) in instance.EmissionLimitConstraint:
                print(p, e, instance.dual[instance.EmissionLimitConstraint[p, e]],
        ...
        for p in instance.time_optimize:
            for s in instance.time_season:
                for tod in instance.time_of_day:
                    print(p, s, tod, instance.dual[instance.DemandConstraint[p, s, tod, c]],
...
def sensitivity_api(instance, techs, algorithm=None):
    ...
    for t in techs:
        ...
        for v in vintages:
            ...
            clb, cub = c.solution.sensitivity.objective(target_var)
            if cub > 1E5:
                cub = 0  # Infinity
        ...
        for v in vintages:
            ...
            tmp_beic_cs = value(instance.CostInvest[t, v]
                                ) if deployed else bic_s[t][vintages.index(v)]
