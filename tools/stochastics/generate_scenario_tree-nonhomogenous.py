...
node_count = 0
def stringify(x): return ', '.join(str(i) for i in x)
...
class Param (object):
    ...
    def __init__(self, **kwargs):
        ...
        if isinstance(pindex, _SetProduct):
            ...
            def skeys(): return (' '.join(str(i) for i in k) for k in self.model_keys)

            keys = param.sparse_keys()
            def f(x): return x[pidx] == spoint

            def r(x): return tuple(x[0:pidx] + x[pidx+1:])
            ...
            def skeys(): return (' '.join(str(i) for i in self.model_keys))
...
def main():
    ...
    begin = clock()
    def duration(): return clock() - begin
