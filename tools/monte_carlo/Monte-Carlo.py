...
def evaluate(param_values, k):
    ...
    cur.execute(
        "SELECT emissions_comm, SUM(emissions) FROM Output_Emissions WHERE emissions_comm='co2' and tech='IMPELCNGA'")
    output_query = cur.fetchall()
    MonteCarlo_Objectives.append(output_query[-1][-1])

    cur.execute(
        "SELECT emissions_comm, SUM(emissions) FROM Output_Emissions WHERE emissions_comm='co2' and tech LIKE 'IMPELCCOA%'")
    output_query = cur.fetchall()
    MonteCarlo_Objectives.append(output_query[-1][-1])
...
MonteCarlo_Objectives = Parallel(n_jobs=num_cores)(delayed(evaluate)(
    param_values[ii, :], ii) for ii in range(0, Number_of_Simulations))
